...
{
	g_versionStr = new wchar_t[MAX_PATH];
	wsprintf( g_versionStr, L"%ws v%ws compiled %ws %ws", dbgGetPluginName(), DBGEXT_VERSION, _T( __DATE__ ), _T( __TIME__ ) );
	g_copyrightStr = new wchar_t[COPYRIGHT_BUF_LEN];
	wsprintf( g_copyrightStr, L"Copyright (C) 2014 Ferrit <ferrit.rce@gmail.com>" );
	...
	wcscat_s( g_copyrightStr, COPYRIGHT_BUF_LEN, L"\n\nSpecial thanks and greetings to Oleh Yuschuk, Cenzo, HellSp@wn, Teddy Rogers, Sendersu, BeaEngine and XED Team!" );
}
