...
void ntGlobalFlagPreHook( bool protect )
{
	...
	const wchar_t* pFileName = PathFindFileName( dbgGetProcessName() );
...
void ntGlobalFlagApply( bool protect )
{
	if( protect )
	{
		...
		if( !remotePatchApply( dbgGetProcessHandle(), (void*)( peb + osGetNtGlobalFlagOffset() ), g_orig, g_patch, sizeof( g_patch ) ) )
			return;
	...
	else
	{
		...
		if( !remotePatchApply( dbgGetProcessHandle(), (void*)( peb + osGetNtGlobalFlagOffset() ), NULL, g_orig, sizeof( g_orig ) ) )
			return;
