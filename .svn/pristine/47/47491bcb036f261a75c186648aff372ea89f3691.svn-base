...
#define ASSEMBLER_CALC_RELATIVE_ADDRESS( actualGlobalEip, instrSize, targetGlobalEip ) \
	( (DWORD)( targetGlobalEip ) - ( (DWORD)( actualGlobalEip ) + (DWORD)( instrSize ) ) );

#define ASSEMBLE( cmd, localEip ) \
{ \
	DWORD codeLen = 0; \
	wchar_t assemblyError[MAX_PATH] = { 0 }; \
	codeLen = assemblerAssemble( cmd, localEip, assemblyError ); \
...
#define ASSEMBLER_ADD_PADDING_AND_STEP( paddingSize, localEip ) \
{ \
	if( paddingSize ) \
	{ \
		DWORD paddingBytes = localEip % paddingSize; \
		if( paddingBytes ) \
		{ \
			memset( (void*)&g_asmBuffer[localEip], 0, paddingBytes ); \
			localEip += paddingBytes; \
		} \
	} \
}
...
#define ASSEMBLER_ADD_DWORD_AND_STEP( data, localEip ) \
{ \
