...
#define PROCNAME ( L"QueryPerformanceCounter" )
...
void queryPerformanceCounterApply( bool protect )
{
	if( protect )
	{
		...
		ASSEMBLE_AND_STEP( L"add edx imm:00000004", localEip );
		...
		ASSEMBLE_AND_STEP( L"adc mem4:-,-,-,00000000 imm:00000000", localEip );
		...
		ASSEMBLER_ADD_DWORD_AND_STEP( li.LowPart, localEip );
		ASSEMBLER_SET_LABEL( "counterHighAddress", localEip );
		ASSEMBLER_ADD_DWORD_AND_STEP( li.HighPart, localEip );
		...
		labelEip = ASSEMBLER_GET_LABEL( "counterHighAddress" );
		wsprintf( cmd, L"adc mem4:-,-,-,%08X imm:00000000", codeBase + labelEip );
		ASSEMBLE( cmd, patchEip );
