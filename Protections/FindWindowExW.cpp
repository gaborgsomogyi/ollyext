...
#define PROCNAME ( L"FindWindowExW" )
...
void findWindowExWApply( bool protect )
{
	if( protect )
	{
		...
		ASSEMBLE_AND_STEP( L"mov/16 ax mem2:esi", localEip );
		ASSEMBLE_AND_STEP( L"mov/16 dx mem2:edi", localEip );
		ASSEMBLE_AND_STEP( L"add esi imm:00000002", localEip );
		ASSEMBLE_AND_STEP( L"add edi imm:00000002", localEip );
		ASSEMBLE_AND_STEP( L"and/16 ax imm:FFDF", localEip );
		ASSEMBLE_AND_STEP( L"cmp/16 ax dx", localEip );
		ASSEMBLER_SET_LABEL( "notSameClassPatch", localEip );
		ASSEMBLE_AND_STEP( L"jnz brdisp:00000000", localEip );
		ASSEMBLE_AND_STEP( L"cmp/16 ax imm:0000", localEip );
