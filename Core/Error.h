...
#define UNABLE_TO_READ_PROTECTION_AT ( L"Unable to read memory protection at: %08X" )
...
#define UNABLE_TO_UNLOAD_MODULE ( L"Unable to unload module %s" )
...
#define UNABLE_TO_DISASSEMBLE_CODE ( L"Unable to disassemble at: %08X error: %s" )
...
#define UNABLE_TO_GET_WIN_VERSION ( L"Unable to get windows version information." )
#define UNABLE_TO_OPEN_REGISTRY_KEY ( L"Unable to open registry key: %s error: %08X" )
#define UNABLE_TO_OPEN_OR_CREATE_REGISTRY_KEY ( L"Unable to open or create registry key: %s error: %08X" )
#define UNABLE_TO_QUERY_REGISTRY_KEY ( L"Unable to query registry key: %s name: %s error: %08X" )
#define INVALID_REGISTRY_KEY_TYPE ( L"Invalid registry keyType key: %s name: %s actual: %d expected: %d error: %08X" )
...
#define UNABLE_TO_GET_DATA_DUMP ( L"Unable to get data window dump" )
#define UNABLE_TO_READ_REMOTE_PROCESS_MEMORY ( L"Unable to read remote process memory at %08X with size %d" )
#define UNABLE_TO_CONVERT_WCHAR_TO_CHAR ( L"Unable to convert string: %s" )
#define UNABLE_TO_FIND_TARGET_ADDRESS ( L"Unable to find target jump address at %08X in command\"%s\"" )
#define BREAKPOINT_FOUND_IN_ASSEMBLY ( L"Breakpoint mnenomic found in the assembly code" )
#define UNABLE_TO_RIP_DATA ( L"Unable to RIP data. Only Olly2.01 version supported!" )
#define UNABLE_TO_GET_SYSTEM_DIRECTORY ( L"Unable to get system directory" )
#define UNABLE_TO_SET_SYMBOL_SERVER_OPTIONS ( L"Unable to set symbol server options" )
#define UNABLE_TO_INITIALIZE_SYMBOL_SERVER ( L"Unable to initialize symbol server" )
#define UNABLE_TO_CLEANUP_SYMBOL_SERVER ( L"Unable to clean up symbol server" )

#define PLUGIN_CHECK_ERROR( condition, action, errorFormat, ... ) \
{ \
	if( !( condition ) ) \
	{ \
		wchar_t errorStr[MAX_PATH]; \
		wsprintf( errorStr, ( errorFormat ), __VA_ARGS__ ); \
		wchar_t placeStr[MAX_PATH]; \
		wsprintf( placeStr, L"\nFile: %s Line: %d", _T( __FILE__ ), __LINE__ ); \
		wcscat_s( errorStr, MAX_PATH, placeStr ); \
		wchar_t lastErrorStr[MAX_PATH]; \
		wsprintf( lastErrorStr, L"\nResult of GetLastError: %08X", GetLastError() ); \
		wcscat_s( errorStr, MAX_PATH, lastErrorStr ); \
		MessageBox( NULL, errorStr, dbgGetPluginName(), MB_OK | MB_ICONERROR ); \
		action \
	} \
}
